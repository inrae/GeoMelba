...
from ....dictionnaire import( field_order, field_type_line_middle, null, field_incoming_flow, \
    ...
    field_flow_production_water_summer, field_flow_production_mes_summer, field_flow_production_phyto_summer, field_flow_production_relative_summer,\
...
#Functions used to manage the history variables 
...
#Function used to order all land scape elements based on id

def order_feature(parcel_layer, line_layer, start_order, end_order):
    """
    Function used to list all landscape elements and order them, based on variables like:
    ...
    """
    flow_list = []
    # As long as the starting point is different from the end point, we order the elements.
    while start_order != end_order:
        line_layer.selectByExpression("{field_order} = '{order}' ".format(field_order=field_order, order=start_order))
        parcel_layer.selectByExpression("{field_order} = '{order}' ".format(field_order=field_order, order=start_order))
        if len(line_layer.selectedFeatureIds()) == 1:
            if line_layer.selectedFeatures()[0].attributes()[line_layer.fields().indexFromName(field_type_line_middle)] != null:
...
class FlowCalculations_Phyto:
    ...
    def __init__(self, watershed_name=None, practices=None, slope=None, land_cover=None, abatement_type=None,
        ...
        - """
        super(FlowCalculations_Phyto_, self).__init__()
        self.abatement_lat = abatement_lat
    ...
    def prepare_layers(self, parcel_layer, line_layer):
        ...
        if selected_season == "summer":
            ...
            line_layer.addAttribute(
                QgsField(field_outgoing_flow_line_to_parcel_summer, QVariant.Double, "double", 10, 2))
        ...
        elif selected_season == "summer and winter":
        ...
        for f in line_layer.getFeatures():
            ...
            line_layer.changeAttributeValue(f.id(), line_layer.fields().indexFromName(field_incoming_flow_from_parcel),
                                            0)
        ...
        if season == 'summer':
            ...
            for f in parcel_layer.getFeatures():
                ...
                if parcel_type in self.production_type:
                    ...
                    prod_phyto_summer = parcel_inflow_production_summer(parcel_type, label, slope, area, self.slope,
                                                                        self.production_phyto)
                    prod_phyto_winter = parcel_inflow_production_winter(parcel_type, label, slope, area, self.slope,
                                                                        self.production_phyto)
                else:
                    prod = 0
                    prod_phyto = 0
                ...
                parcel_layer.changeAttributeValue(f.id(),
                                                  parcel_layer.fields().indexFromName(field_incoming_flow_summer), 0)
                parcel_layer.changeAttributeValue(f.id(),
                                                  parcel_layer.fields().indexFromName(field_incoming_flow_winter), 0)
                parcel_layer.changeAttributeValue(f.id(), parcel_layer.fields().indexFromName(field_outgoing_flow), 0)
                parcel_layer.changeAttributeValue(f.id(),
                                                  parcel_layer.fields().indexFromName(field_outgoing_flow_summer), 0)
                parcel_layer.changeAttributeValue(f.id(),
                                                  parcel_layer.fields().indexFromName(field_outgoing_flow_winter), 0)
                parcel_layer.changeAttributeValue(f.id(),
                                                  parcel_layer.fields().indexFromName(field_parcel_outflow_drain), 0)
                parcel_layer.changeAttributeValue(f.id(), parcel_layer.fields().indexFromName(
                    field_parcel_outflow_drain_summer), 0)
                parcel_layer.changeAttributeValue(f.id(), parcel_layer.fields().indexFromName(
                    field_parcel_outflow_drain_winter), 0)
                parcel_layer.changeAttributeValue(f.id(), parcel_layer.fields().indexFromName(field_flow_production),
                                                  prod)
                parcel_layer.changeAttributeValue(f.id(),
                                                  parcel_layer.fields().indexFromName(field_flow_production_water),
                                                  prod_water)
                parcel_layer.changeAttributeValue(f.id(),
                                                  parcel_layer.fields().indexFromName(field_flow_production_mes),
                                                  prod_mes)
                parcel_layer.changeAttributeValue(f.id(),
    ...
    def line_analysis(self, line_layer, parcel_layer, connexion_layer, line_id, history_field, history_abatement_field):
        ...
        # The linear with river type and linear which can impact water flow lengthwise can both have inflow from linear above.
        ...
        if line_type == drain and active_drain == 1:
            ...
            abatement_coefficient = coefficient_abatement_longitudinal_TE(drain, line_length, self.abatement_long)
    ...
    def get_inflow_from_parcel(self, line, line_type, line_layer, parcel_layer, connexion_layer, history_field,
        ...
        if line_type != river and line_type != drain:
            ...
            dict_history = {}
            dict_history_abat = {}
            # From the connexion_layer we can get the coefficient of flow between the two parcels and the id of the
            # parcel above.
    ...
    def ecoulement_bv(self, parcel_layer, line_layer, connexion_layer, count, element, *args):
        global history_field, history_abatement_field, history
        if element == 2:
    ...
    def ecoulement_emis(self, parcel_layer, line_layer, connexion_layer, count, element):
        global history_field, history_abatement_field
        if element == 0:
    ...
    def ecoulement_recu(self, parcel_layer, line_layer, connexion_layer, count, element, quick_value):
        ...
        global history_field, history_abatement_field, layer, feature
        if element == 2:
            self.abatement = self.abatement_phyto
            self.production = self.production_phyto
            self.abatement_long = self.abatement_long_phyto
...
class FlowCalculations_MES:
    ...
    def __init__(self, watershed_name=None, practices=None, slope=None, land_cover=None, abatement_type=None,
        ...
        """Class attributes are :
        - """
        super(FlowCalculations_MES, self).__init__()
        self.watershed_name = watershed_name
        self.practices = practices
        self.slope = slope
...
#prepare the attributes of tableau attributaire
    ...
    def prepare_layers(self, parcel_layer, line_layer):
        ...
        line_layer.addAttribute(QgsField(field_outgoing_flow_long_winter, QVariant.Double, "double", 10, 2))
        line_layer.addAttribute(QgsField(field_outgoing_flow_lat, QVariant.Double, "double", 10, 2))
        line_layer.addAttribute(QgsField(field_outgoing_flow_lat_summer, QVariant.Double, "double", 10, 2))
        line_layer.addAttribute(QgsField(field_outgoing_flow_lat_winter, QVariant.Double, "double", 10, 2))
        line_layer.addAttribute(QgsField(field_outgoing_flow_line_to_parcel, QVariant.Double, "double", 10, 2))
        ...
        line_layer.addAttribute(QgsField(field_incoming_flow_rating, QVariant.Int, "int", 2))
        line_layer.addAttribute(QgsField(field_incoming_flow_rating_summer, QVariant.Int, "int", 2))
        line_layer.addAttribute(QgsField(field_incoming_flow_rating_winter, QVariant.Int, "int", 2))
        line_layer.addAttribute(QgsField(field_outgoing_flow_long_up, QVariant.Double, "double", 10, 5))
        line_layer.addAttribute(QgsField(field_outgoing_flow_long_up_summer, QVariant.Double, "double", 10, 5))
        line_layer.addAttribute(QgsField(field_outgoing_flow_long_up_winter, QVariant.Double, "double", 10, 5))
        line_layer.addAttribute(QgsField(field_outgoing_flow_long_down, QVariant.Double, "double", 10, 5))
        ...
        for f in parcel_layer.getFeatures():
            ...
            if parcel_type in self.production_type:
                ...
                prod_mes_summer = parcel_inflow_production_summer(parcel_type, label, slope, area, self.slope,
                                                    self.production_mes)*longueur 
                prod_mes_winter = parcel_inflow_production_winter(parcel_type, label, slope, area, self.slope,
                                                    self.production_mes)*longueur
            else:
                prod = 0
                prod_mes = 0
            ...
            parcel_layer.changeAttributeValue(f.id(), parcel_layer.fields().indexFromName(
                field_incoming_flow_summer), 0)
            parcel_layer.changeAttributeValue(f.id(), parcel_layer.fields().indexFromName(
                field_incoming_flow_winter), 0)
            parcel_layer.changeAttributeValue(f.id(), parcel_layer.fields().indexFromName(
                field_outgoing_flow), 0)
            parcel_layer.changeAttributeValue(f.id(), parcel_layer.fields().indexFromName(
                field_outgoing_flow_summer), 0)
            parcel_layer.changeAttributeValue(f.id(), parcel_layer.fields().indexFromName(
                field_outgoing_flow_winter), 0)
            parcel_layer.changeAttributeValue(f.id(), parcel_layer.fields().indexFromName(
    ...
    def ecoulement_emis(self, parcel_layer, line_layer, connexion_layer, count, element):
        ...
        parcelle_id_selected = [
            parcel_layer.selectedFeatures()[0].attributes()[parcel_layer.fields().indexFromName(field_parcel_id)]]
        args = None
        self.prepare_layers(parcel_layer, line_layer)
        ...
        parcel_layer.addAttribute(QgsField(field_parcel_rating, QVariant.Int, "int", 2))
        parcel_layer.addAttribute(QgsField(field_feature_selected, QVariant.Int, "int", 1))
        parcel_layer.changeAttributeValue(parcelle_id_selected[0],
                                          parcel_layer.fields().indexFromName(field_feature_selected), 1)
        for parcelle in parcel_layer.getFeatures():
            parcel_layer.changeAttributeValue(parcelle.id(), parcel_layer.fields().indexFromName(field_parcel_rating),
                                              99)
        parcel_layer.commitChanges()
        parcel_layer.triggerRepaint()
        ...
        list_id_parcelle = [
            parcel_layer.selectedFeatures()[0].attributes()[parcel_layer.fields().indexFromName(field_parcel_id)]]
        selection = [str(parcel_layer.selectedFeatures()[0].id())]
        if parcel_layer.selectedFeatures()[0].attributes()[
            parcel_layer.fields().indexFromName(field_type_parcel)] in self.production_type:
            while len(selection) > 0:
                for selected_feature in selection:
                    selection.remove(selected_feature)
                    f = parcel_layer.getFeature(int(selected_feature))
                    ...
                    for feat in connexion_layer.getFeatures(QgsFeatureRequest(select_connexion)):
                        attrs = feat.attributes()
                        uh_dwn = attrs[connexion_layer.fields().indexFromName(field_connexions_parcel_below)]
                        if uh_dwn != NULL:
                            if uh_dwn not in list_id_parcelle:
                                list_id_parcelle.append(uh_dwn)
                                selection.append(uh_dwn)
        ...
        parcel_layer.startEditing()
        for parcel_id in list_id_parcelle:
            parcel_layer.changeAttributeValue(parcel_id, parcel_layer.fields().indexFromName(field_incoming_flow), 0)
        parcel_layer.commitChanges()
        parcel_layer.triggerRepaint()

        n = parcel_layer.selectedFeatures()[0].attributes()[parcel_layer.fields().indexFromName(field_order)]
        limit = parcel_layer.featureCount() + line_layer.featureCount() + 1
        list_ecoulement = order_feature(parcel_layer, line_layer, n, limit)
        processed_poly = []
        parcel_abat = []
        ...
        for f_id in list_ecoulement:
            ...
            if f_id.split('_')[0] == 'line':  # Selection of linear element in the list of parcel and linear.
                self.line_analysis(line_layer, parcel_layer, connexion_layer, id_split, history_field,
                                   history_abatement_field)
            if f_id.split('_')[0] == 'parcel':  # Selection of parcel element in the list of parcel and linear.
                processed_poly, parcel_abat = self.parcel_analysis(line_layer, parcel_layer, connexion_layer, id_split,
                                                                   parcelle_id_selected, list_id_parcelle, parcel_abat,
                                                                   processed_poly, history_field,
                                                                   history_abatement_field)
        line_layer.commitChanges()
        line_layer.triggerRepaint()
        parcel_layer.commitChanges()
        ...
        prefix = str(count) + '_' + str(parcelle_id_selected[0])
        parcel_layer_transfert, layer_lineaire_transfert, riviere_layer, parcel_layer_abattement, layer_lineaire_abattement, max_entrant_parcelle, max_entrant_lineaire, max_entrant_riviere = self.create_layers_emit(
            args, parcel_layer, line_layer, processed_poly,
            group_outgoing_flow_parcel + str(count) + " " + str(parcelle_id_selected[0]),
            group_abatement_analysis + str(count) + ' ' + str(parcelle_id_selected[0]),
            group_flow_transfer + str(count) + ' ' + str(parcelle_id_selected[0]), prefix)
        ...
        return parcel_layer_transfert, layer_lineaire_transfert, riviere_layer, parcel_layer_abattement, layer_lineaire_abattement
    ...
    def ecoulement_recu(self, parcel_layer, line_layer, connexion_layer, count, element, quick_value):
        ...
        for parcelle in parcel_layer.getFeatures():
            parcel_layer.changeAttributeValue(parcelle.id(),
                                              parcel_layer.fields().indexFromName(field_flow_production_rating), 99)
            parcel_layer.changeAttributeValue(parcelle.id(),
                                              parcel_layer.fields().indexFromName(field_flow_abatement_rating), 999)
            parcelle_id_selected.append(parcelle.attributes()[parcel_layer.fields().indexFromName(field_parcel_id)])
            list_id_parcel.append(parcelle.attributes()[parcel_layer.fields().indexFromName(field_parcel_id)])
        n = 0
        list_ecoulement = order_feature(parcel_layer, line_layer, n, limit)
            ...
            for f_id in list_ecoulement:
                ...
                if f_id.split('_')[0] == 'line':  # Selection of linear element in the list of parcel and linear.
                    self.line_analysis(line_layer, parcel_layer, connexion_layer, id_split, history_field,
                                       history_abatement_field)
                if f_id.split('_')[0] == 'parcel':  # Selection of parcel element in the list of parcel and linear.
                    processed_poly, parcel_abat = self.parcel_analysis(line_layer, parcel_layer, connexion_layer,
                                                                       id_split, parcelle_id_selected, list_id_parcel,
                                                                       parcel_abat, processed_poly, history_field,
                                                                       history_abatement_field)
            line_layer.commitChanges()
            line_layer.triggerRepaint()
        ...
        if line_layer.fields().indexFromName(field_flow_abatement_rating) != -1:
            line_layer.deleteAttribute(line_layer.fields().indexFromName(field_outlet_inflow))
            line_layer.deleteAttribute(line_layer.fields().indexFromName(field_flow_abatement_rating))
            line_layer.deleteAttribute(line_layer.fields().indexFromName(field_flow_abatement))
        line_layer.addAttribute(QgsField(field_outlet_inflow, QVariant.Double, "double", 10, 2))
        line_layer.addAttribute(QgsField(field_flow_abatement_rating, QVariant.Int, "int", 3))
        line_layer.addAttribute(QgsField(field_flow_abatement, QVariant.Double, "double", 10, 2))
        ...
        parcel_layer.startEditing()
        parcel_layer.deleteAttribute(parcel_layer.fields().indexFromName(field_flow_production_relative))
        parcel_layer.deleteAttribute(parcel_layer.fields().indexFromName(field_flow_production_rating))
        parcel_layer.deleteAttribute(parcel_layer.fields().indexFromName(field_flow_abatement_rating))
        parcel_layer.deleteAttribute(parcel_layer.fields().indexFromName(field_feature_selected))
        parcel_layer.deleteAttribute(parcel_layer.fields().indexFromName(field_flow_abatement))
        parcel_layer.commitChanges()
        parcel_layer.triggerRepaint()
        ...
        parcel_layer.startEditing()
        parcel_layer.addAttribute(QgsField(field_flow_production_relative, QVariant.Double, "double", 5, 2))
        parcel_layer.addAttribute(QgsField(field_flow_production_rating, QVariant.Int, "int", 2))
        parcel_layer.addAttribute(QgsField(field_flow_abatement_rating, QVariant.Int, "int", 2))
        parcel_layer.addAttribute(QgsField(field_feature_selected, QVariant.Int, "int", 1))
        parcel_layer.addAttribute(QgsField(field_flow_abatement, QVariant.Double, "double", 10, 2))
        parcel_layer.changeAttributeValue(parcelle_id_select,
                                          parcel_layer.fields().indexFromName(field_feature_selected), 1)
        parcel_layer.commitChanges()
        parcel_layer.triggerRepaint()
...
class FlowCalculations_EAU:

    def __init__(self, watershed_name=None, practices=None, slope=None, land_cover=None, abatement_type=None,
                 production_type=None, abatement=None, abatement_water=None,
                 production=None, production_water=None, line_type=None,
                 abatement_type_long=None, abatement_type_lat=None, abatement_long=None,abatement_long_water=None, abatement_lat=None, abatement_lat_water=None,
               output_path=None):

        super(FlowCalculations_EAU, self).__init__()
        self.watershed_name = watershed_name
        self.practices = practices
        self.slope = slope
        ...
        self.abatement_water_winter = abatement_water_winter
        self.abatement_water_winter = abatement_water_wo
        self.abatement_mes_summer = abatement_mes_summer
        self.production_type = production_type
        self.production = None
    ...
    def prepare_layers(self, parcel_layer, line_layer):
        ...
        for f in parcel_layer.getFeatures():
            ...
            if parcel_type in self.production_type:
                ...
                prod_water_summer = parcel_inflow_production_summer(parcel_type, label, slope, area, self.slope,
                                                      self.production_water)
                prod_water_winter = parcel_inflow_production_winter(parcel_type, label, slope, area, self.slope,
                                                      self.production_water)
